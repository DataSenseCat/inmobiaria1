          ...
          <div className="flex justify-between items-center text-sm">
            <div className="flex items-center space-x-6">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-gray-700">{import.meta.env.VITE_CONTACT_PHONE || '+54 383 490-1545'}</span>
              </div>
              <div className="flex items-center space-x-2">
                <Mail className="h-4 w-4 text-gray-600" />
                <span className="text-gray-700">{import.meta.env.VITE_CONTACT_EMAIL || 'info@inmobiliariacatamarca.com'}</span>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              ...
              {user ? (
                <div className="flex items-center space-x-3">
                  <Link to="/favoritos" className="flex items-center space-x-1 text-gray-700 hover:text-gray-900">
                ...
                <Link to="/auth/sign-in" className="flex items-center space-x-1 text-gray-700 hover:text-gray-900">
        ...
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden lg:flex items-center space-x-6">
              ...
              <div className="relative group">
                ...
                {propertiesDropdownOpen && (
                  <div
                    ...
                    onMouseLeave={() => setPropertiesDropdownOpen(false)}
                  >
                    <Link to="/propiedades" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                      Todas las Propiedades
                    </Link>
                    <Link to="/propiedades?operation=venta" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                      En Venta
                    </Link>
                    <Link to="/propiedades?operation=alquiler" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                      En Alquiler
                    </Link>
                    <Link to="/propiedades?operation=temporal" className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                      Alquiler Temporario
                    </Link>
                  </div>
                )}
              ...
              <Link to="/emprendimientos" className="text-gray-700 hover:text-gray-900 font-medium transition-colors">
                Emprendimientos
              </Link>

              <Link to="/tasaciones" className="text-gray-700 hover:text-gray-900 font-medium transition-colors">
                Tasaciones
              </Link>

              <Link to="/la-empresa" className="text-gray-700 hover:text-gray-900 font-medium transition-colors">
                La Empresa
              </Link>

              <Link to="/contacto" className="text-gray-700 hover:text-gray-900 font-medium transition-colors">
                Contacto
              </Link>

              {/* Admin Link */}
              {user && isAdmin && (
                <Link to="/admin" className="flex items-center space-x-1 text-blue-600 hover:text-blue-800 font-medium transition-colors">
          ...
          <div className="lg:hidden bg-white border-t">
            <div className="container mx-auto px-4 py-4">
              <nav className="space-y-4">
                ...
                <Link
                  to="/propiedades?operation=temporal"
                  className="block text-gray-600 hover:text-gray-800 ml-4"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Alquiler Temporario
